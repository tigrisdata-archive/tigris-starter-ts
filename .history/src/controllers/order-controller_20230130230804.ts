...
export class OrderController implements Controller {
  ...
  public async createOrder(req: Request, res: Response, next: NextFunction) {
    ...
    const user = await this.users.findOne({ filter: { userId: order.userId } });
    const product = await this.products.findOne({
      filter: { productId: order.productId },
    });
    const userOrders = await this.orders.findOne({filter: { userId: order.userId}});
    if(userOrders){
        const orderTotal = userOrders.productItems.
    }
  }
